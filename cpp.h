...
static inline double dayLength(double latitude, int dofy){
  ...
  double tmp2 = tmp1 / R_pow(1 - R_pow_di(tmp1,2), 0.5);
...
static inline void relSolRad(double dlength, 
  ...
  int sunrise = fround(12 - 0.5 * dlength, 0);
  int sunset = fround(12 + 0.5 * dlength, 0);
...
static inline double leafArea(double leaf_number,
  ...
  for(int i = 1; i <= ceil(l_num); ++i){
    ...
    if(i >= floor(l_num))
      pla += amax * exp( -0.0344 * R_pow_di(i - lnm, 2) + 0.000731 * R_pow_di(i - lnm, 3)) *
...
static inline double evaporation(double dyse, 
  ...
  double delt = exp(21.255 - 5304 / (273 + t_tmean)) * (5304 / R_pow_di(273 + t_tmean, 2));
